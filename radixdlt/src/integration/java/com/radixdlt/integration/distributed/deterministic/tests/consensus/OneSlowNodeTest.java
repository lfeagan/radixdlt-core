...
public class OneSlowNodeTest {
	...
	public void when_three_fast_nodes_and_one_slow_node_two_cycles__then_missing_parent_should_not_cause_sync_exception() {
		...
		for (int curLeader = 1; curLeader <= 2; curLeader++) {
			processingSequence.add(Pair.of(ChannelId.of(1, curLeader), ViewTimeout.class));
			processingSequence.add(Pair.of(ChannelId.of(2, curLeader), ViewTimeout.class));
			processingSequence.add(Pair.of(ChannelId.of(3, curLeader), ViewTimeout.class));
